...
//		gruppenverschieber = Ressourcen.getGruppenVerschieber();
		...
		try {
			/*
			 * Was: setPage("file:///" + config.getConfig("KiGaLoggingPath")
			 *                        + "/titlepage/kiga.htm")
			 *
			 * KiGaLoggingPath ships empty, so that produced
			 * "file:////titlepage/kiga.htm" - an absolute path at the filesystem root.
			 * The title picture therefore never appeared and the user was greeted by a
			 * modal "no online help" error dialog on every start. The page and its two
			 * images are packaged in the jar, so a filesystem path could not reach them
			 * at all. KigaResources looks in the configured installation directory
			 * first, if one is set, and otherwise on the classpath.
			 */
			...
			allgemeinHTML.setPage(helpPage);
		} catch (Exception e1) {
...
//		int columns = 0;
	...
	public void addKind(int id, String vorname, String nachname,
		...
		ergebnisModel.addRow(new Object[] { Integer.valueOf(id), vorname, nachname,
